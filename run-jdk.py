...
def parse_args() -> Tuple[Commands, BuildArgs, RunArgs]:
    ...
    optional.add_argument('--gdb', action='store_true', default=False, help="Launch with GDB")
    optional.add_argument('--rr', action='store_true', default=False, help="Launch with rr record")
    ...
    optional.add_argument('--compressed-oops', action='store_true', default=False, help="UseCompressedOops")
    ...
    build_args = BuildArgs(profile=args.profile, exploded=args.exploded, bundle=args.build_id is not None, features=args.features, build_id=args.build_id)
    run_args = RunArgs(gc=args.gc, bench=args.bench, heap=args.heap, iter=args.iter, noc1=args.no_c1, noc2=args.no_c2, gdb=args.gdb, rr=args.rr, exploded=args.exploded, threads=args.threads, mu=args.mu, jdk_args=args.jdk_args, compressed_oops=args.compressed_oops)
...
def config(profile: str):
    exec(f'sh configure --disable-zip-debug-info --disable-warnings-as-errors --with-debug-level={profile} --with-target-bits=64 --with-jvm-features=shenandoahgc', cwd=OPENJDK)
...
def build(build_args: BuildArgs):
    if build_args.exploded:
        ...
        exec(f'rm -f {OPENJDK}/build/linux-x86_64-normal-server-{build_args.profile}/bundles/*', cwd=MMTK_DEV)
    features = f'GC_FEATURES={",".join(build_args.features)} 'if build_args.features is not None else ''
    exec(f'make --no-print-directory CONF=linux-x86_64-normal-server-{build_args.profile} THIRD_PARTY_HEAP={MMTK_OPENJDK}/openjdk {target} {features}', cwd=OPENJDK)
...
def run(build_args: BuildArgs, run_args: RunArgs):
    ...
    elif run_args.noc1: compiler_args += '-XX:-TieredCompilation'
    elif run_args.noc2: compiler_args += '-XX:TieredStopAtLevel=1'
    # GDB Wrapper
    if run_args.gdb:
        debugger_wrapper = 'gdb --args'
    elif run_args.rr:
        debugger_wrapper = f'rr record'
    else:
        debugger_wrapper = ''
    # Benchmark args
    bm_args = f''
    if run_args.mu is not None: bm_args += f' -t {run_args.mu}'
    # Extra
...
if commands.kill: kill()
if commands.config: config(profile=build_args.profile)
if commands.clean: clean(profile=build_args.profile)
if commands.build: build(build_args)
if commands.run: run(build_args, run_args)
