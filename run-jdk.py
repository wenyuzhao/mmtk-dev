...
def config(profile: str):
    exec(f'sh configure --disable-warnings-as-errors --with-debug-level={profile} --with-target-bits=64 --with-jvm-features=shenandoahgc', cwd=OPENJDK)
...
def run(profile: str, gc: str, bench: str, heap: Optional[str], iter: int, noc1: bool, noc2: bool, gdb: bool, rr: bool, exploded: bool, threads: Optional[int], mu: Optional[int], jdk_args: Optional[str]):
    # MMTk or HotSpot GC args
    if gc in HOTSPOT_GCS:
        hs_gc = HOTSPOT_GCS[gc] + f' -agentpath:{PROBES}/libperf_statistics.so'
        mmtk_env_args = f'RUST_BACKTRACE=1 LD_PRELOAD={PROBES}/libperf_statistics.so'
    else:
        hs_gc = '-XX:+UseThirdPartyHeap'
        mmtk_env_args = f'RUST_BACKTRACE=1 MMTK_PLAN={gc}'
    if threads is not None: mmtk_env_args += f' MMTK_THREADS={threads}'
    # Heap size
    heap_size = f'-Xms{heap} -Xmx{heap}' if heap is not None else ''
    heap_args = f'-XX:MetaspaceSize=1G {heap_size}'
    # Probes args
    ...
    extra_jdk_args += ' -XX:+UnlockExperimentalVMOptions -XX:-UseCompressedOops -XX:-UseCompressedClassPointers'
    # Run
    ...
    exec(f'{mmtk_env_args} {debugger_wrapper} {java} {extra_jdk_args} {hs_gc} {heap_args} {compiler_args} {probe_args} Harness -n {iter} -c probe.DacapoChopinCallback {bench} {bm_args}', cwd=MMTK_DEV)
...
run(profile=args.profile, gc=args.gc, bench=args.bench, heap=args.heap, iter=args.iter, noc1=args.no_c1, noc2=args.no_c2, gdb=args.gdb, rr=args.rr, exploded=args.exploded, threads=args.threads, mu=args.mu, jdk_args=args.jdk_args)
