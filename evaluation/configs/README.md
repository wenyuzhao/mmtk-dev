...
1. Install the wrapper script: `uv sync --locked`
2. Produce JDK builds: `uv run mmtk-jdk bench build --config lxr-pldi/xput`
3. Rsync all the necessary files to a evaluation machine: `uv run mmtk-jdk bench rsync --remote bear.moma`
4. **On `bear.moma`:** Start evaluation: `uv run mmtk-jdk bench run --config lxr-pldi/xput`
